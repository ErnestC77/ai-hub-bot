...
async def test_success_reserves_settles_and_returns_result(session, fake_redis, monkeypatch):
    user = await _seed(session, _model())  # цена 1 -> оценка 4 кредита (маржа 30%)
    ...
    assert request.estimated_credits == 4
...
async def test_expensive_estimate_without_confirm_raises_confirmation(session, fake_redis, monkeypatch):
    user = await _seed(
        session, _model(code="exp", price=35, min_credits=20, recommended=30),
    ...
    assert exc_info.value.estimated_credits == 138  # оценка 2000/1000 при цене 35 (маржа 30%)
...
async def test_insufficient_balance_rolls_back_pending_request(session, monkeypatch):
    user = await _seed(session, _model(), balance=3)  # оценка 4 > 3
    ...
    assert fetched.credits_balance == 3
...
async def test_more_expensive_fallback_without_confirm_raises_confirmation(session, monkeypatch):
    ...
    assert exc_info.value.estimated_credits == 4  # оценка по fallback-модели (цена 1, маржа 30%)
...
async def test_free_tier_cap_blocks_when_estimate_exceeds_remaining(session, fake_redis, monkeypatch):
    # cap=100, spent=97, оценка cheap-модели = 4 -> 97 + 4 > 100.
    user = await _seed(session, _model(), purchased=0, spent=97)
    monkeypatch.setattr(tgs, "_provider", FakeProvider())
